...
bool AnnounceEvent::configureRaidEvent(xmlNodePtr eventNode)
{
	...
	if(readXMLString(eventNode, "type", strValue))
	{
		...
		{
			m_messageType = MSG_EVENT_ADVANCE;
			std::cout << "[Notice AnnounceEvent::configureRaidEvent]: Unknown type tag missing for announce event: " << strValue << ". Using default: " << (int32_t)m_messageType << std::endl;
		}
	}
	else
	{
...
bool SingleSpawnEvent::executeEvent()
{
	...
	if(!monster)
	{
		std::cout << "[Error SingleSpawnEvent::executeEvent]: Cant create monster " << m_monsterName << std::endl;
		return false;
	}
	...
	{
		delete monster;
		std::cout << "[Error SingleSpawnEvent::executeEvent]: Cant place monster " << m_monsterName << std::endl;
		return false;
	}
...
bool ScriptEvent::executeEvent()
{
	...
	{
		std::cout << "[Error ScriptEvent::executeEvent]: Call stack overflow. ScriptEvent::executeEvent" << std::endl;
		return 0;
	}
}
