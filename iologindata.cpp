...
bool IOLoginData::getAccountId(const std::string& name, uint32_t& number)
{
	...
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `id` FROM `accounts` WHERE `name` " << db->getStringComparisonOperator() << " " << db->escapeString(name) << " LIMIT 1;";
...
bool IOLoginData::hasFlag(uint32_t accId, PlayerFlags value)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `group_id` FROM `accounts` WHERE `id` = " << accId << " LIMIT 1;";
...
bool IOLoginData::hasFlag(PlayerFlags value, const std::string& accName)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `group_id` FROM `accounts` WHERE `name` " << db->getStringComparisonOperator() << " " << db->escapeString(accName) << " LIMIT 1;";
...
bool IOLoginData::accountExists(uint32_t accId)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `id` FROM `accounts` WHERE `id` = " << accId << " LIMIT 1;";
...
bool IOLoginData::getPassword(uint32_t accId, const std::string& name, std::string& password)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `password` FROM `accounts` WHERE `id` = " << accId << " LIMIT 1;";
...
bool IOLoginData::validRecoveryKey(uint32_t accountId, const std::string recoveryKey)
{
	...
	query << db->getStringComparisonOperator() << " " << db->escapeString(recoveryKey) << " LIMIT 1;";
...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	query << db->getStringComparisonOperator() << " " << db->escapeString(name) << " AND `world_id` = ";
...
bool IOLoginData::hasFlag(const std::string& name, PlayerFlags value)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `group_id` FROM `players` WHERE `name` " << db->getStringComparisonOperator() << " " << db->escapeString(name) << " AND `deleted` = 0 LIMIT 1;";
...
bool IOLoginData::getGuidByNameEx(uint32_t& guid, bool &specialVip, std::string& name)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `id`, `name`, `group_id` FROM `players` WHERE `name` " << db->getStringComparisonOperator() << " " << db->escapeString(name) << " AND `deleted` = 0 AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " LIMIT 1;";
...
DeleteCharacter_t IOLoginData::deleteCharacter(uint32_t accountId, const std::string characterName)
{
	...
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `id` FROM `players` WHERE `name` " << db->getStringComparisonOperator() << " " << db->escapeString(characterName) << " AND `account_id` = " << accountId << " AND `deleted` = 0 LIMIT 1;";
...
uint32_t IOLoginData::getLastIPByName(const std::string& name) const
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `lastip` FROM `players` WHERE `name` " << db->getStringComparisonOperator() << " " << db->escapeString(name) << " AND `deleted` = 0 LIMIT 1;";
...
uint32_t IOLoginData::getAccountIdByName(const std::string& name) const
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `account_id` FROM `players` WHERE `name` " << db->getStringComparisonOperator() << " " << db->escapeString(name) << " AND `deleted` = 0 LIMIT 1;";
...
bool IOLoginData::getDefaultTownByName(const std::string& name, uint32_t& townId)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `town_id` FROM `players` WHERE `name` " << db->getStringComparisonOperator() << " " << db->escapeString(name) << " AND `deleted` = 0 LIMIT 1;";
