...
bool IOLoginData::isPremium(uint32_t guid)
{
	...
	DBQuery query;
	query << "SELECT `account_id`, `group_id` FROM `players` WHERE `id` = " << guid;
	if(!(result = db->storeQuery(query.str())))
		return false;
