...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preload /*= false*/)
{
	...
	query << "SELECT `id`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `redskulltime`, `redskull`, `guildnick`, `rank_id`, `town_id`, `balance`, `stamina`, `loss_experience`, `loss_mana`, `loss_skills`, `loss_items` FROM `players` WHERE `name`=" << db->escapeString(name);
	if(!(result = db->storeQuery(query.str())))
		return false;
...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert)
{
	...
	for(ItemBlockList::const_iterator it = itemList.begin(); it != itemList.end(); ++it)
	{
		...
		char buffer[attributesSize * 2 + 75]; //MUST be (size * 2), else people can crash server when filling letter with ¥ŒÆÑÓ£
...
bool IOLoginData::hasFlag(std::string name, PlayerFlags value)
{
	...
	query << "SELECT `group_id` FROM `players` WHERE `name` = " << db->escapeString(name);
	if(!(result = db->storeQuery(query.str())))
		return false;
...
bool IOLoginData::getGuidByName(uint32_t &guid, std::string& name)
{
	...
	query << "SELECT `name`, `id` FROM `players` WHERE `name` = " << db->escapeString(name);
	if(!(result = db->storeQuery(query.str())))
		return false;
...
int16_t IOLoginData::deleteCharacter(uint32_t accountNumber, const std::string characterName)
{
	...
	if(!_player)
	{
		query << "SELECT `id` FROM `players` WHERE `name` = " << db->escapeString(characterName) << " AND `account_id` = " << accountNumber;
		if(!(result = db->storeQuery(query.str())))
			return 0;
