		...
		if(offlineTrainingSkill != -1)
		{
			...
			if(offlineTime >= 600)
			{
				...
				if(offlineTrainingTime >= 60)
				{
					...
					{
						int32_t promotedVocationId = Vocations::getInstance()->getPromotedVocation(player->getVocationId());
					...
					else if(offlineTrainingSkill == SKILL__MAGLEVEL)
					{
						...
						player->addOfflineTrainingTries(SKILL__MAGLEVEL, offlineTrainingTime * (vocation->getGainAmount(GAIN_MANA) / gainTicks));
					}
