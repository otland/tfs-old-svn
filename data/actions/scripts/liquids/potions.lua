...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if(health and not doCreatureAddHealth(itemEx.uid, math.random(health[1], health[2]) * config.healthMultiplier)) then
		return false
	end

	local mana = potion.mana
	if(mana and not doPlayerAddMana(itemEx.uid, math.random(mana[1], mana[2]) * config.manaMultiplier)) then
		return false
	end
