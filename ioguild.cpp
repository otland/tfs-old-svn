...
bool IOGuild::getRankIdByGuildIdAndName(uint32_t &rankId, const std::string& rankName, uint32_t& guildId)
{
	...
	DBQuery query;
	DBResult* result;
	query << "SELECT `id` FROM `guild_ranks` WHERE `guild_id` = " << guildId << " AND `name` = " << db->escapeString(rankName);
	if(!(result = db->storeQuery(query.str())))
		return false;
...
bool IOGuild::changeRankName(std::string oldRankName, std::string newRankName, uint32_t guildId)
{
	...
	query << "SELECT `id` FROM `guild_ranks` WHERE `name` = " << db->escapeString(oldRankName) << " AND `guild_id` = " << guildId;
	if(!(result = db->storeQuery(query.str())))
		return false;
