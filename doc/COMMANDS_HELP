...
[ LIST
	...
	/attr
		...
		Values
			command
				* Items only
					...
					action/actionid/aid
					unique/uniqueid/uid
					destination/position/location/pos/dest/loc
			type
				* Items only - set/add/new & erase/remove/delete
					bool/boolean
					int/integer/num/number
					float/double
					string/text/*
			attribute
				* Items - set/add/new & erase/remove/delete
					duration - number
					description - string
					text - string
					date - number
		...
		Examples
			/attr aid 300
			/attr set string text "Hello guys"
			/attr skull 2
	...
	/shutdown
		...
		Parameters
			<optional>time/kill/cancel

		Examples
			/shutdown 3 (Shutdown after 3 minutes)
			/shutdown cancel (Cancel shutdown event)
	...
	/newtype
		Outfit changer.
		Parameters
			lookType/monster/item, <optional>playerName

		Examples
			/newtype 266
			/newtype Demon
			/newtype 2160 (Changes outfit to crystal coin)
	...
	/bc
		...
		Examples:
			/bc Hiho
			/bc green Hello! (This one will be green)
	...
	/save
		...
		Examples
			...
			/save GM Tester (saves only single character)
	...
	/reports
		Displays server reports (these reported by CTRL + Z).
		Parameters
			<optional>reportId
	...
	/mute
		...
		Examples
			/mute Tester (mute forever)
			/mute Tester, 30 (mute for 30 seconds)
	...
	/gethouse
		...
		Examples
			...
			/gethouse GM Tester, yes (get house name and teleport there)
	...
	!q
		...
		NOTE: It isn't counting the money in depots/houses.
