			...
			doPlayerSendCancel(cid, text)
				...
				Example
					if getPlayerLevel(cid) >= 10 then --checking level
						doSummonCreature("Chicken", fromPosition)
					else
...
[ LIST
	...
	stopEvent(eventid)
	addDialog(dialog, dialogId, cid, callback)
	hasPlayerClient(cid)
	...
	debugPrint(text)
	doShutdown()
	doSummonCreature(name, pos[, extend = false[, force = false]])
	getOnlinePlayers()
