...
{
	isLoaded = false;
	houseName = "Forgotten headquater (Flat 1, Area 42)";
	houseOwner = 0;
	posEntry.x = 0;
	posEntry.y = 0;
	posEntry.z = 0;
		...
		if(houseTile && houseTile->getHouse() == this)
		{
			if(getHouseAccessLevel(player) >= getHouseAccessLevel(kickingPlayer) && !kickingPlayer->hasFlag(PlayerFlag_CanEditHouses))
			{
				...
				if(g_game.internalTeleport(kickingPlayer, g_game.getClosestFreeTile(
					player, getEntryPosition()), true) == RET_NOERROR)
				{
...
void House::setAccessList(uint32_t listId, const std::string& textlist)
{
	...
	for(HouseTileList::iterator it = houseTiles.begin(); it != houseTiles.end(); ++it)
	{
		...
		{
			for(CreatureVector::iterator cit = hTile->creatures.begin(); cit != hTile->creatures.end(); ++cit)
			{
	...
	for(KickPlayerList::iterator itkick = kickList.begin(); itkick != kickList.end(); ++itkick)
	{
	...
	if(doc)
	{
		...
		while(houseNode)
		{
			if(xmlStrcmp(houseNode->name,(const xmlChar*)"house") == 0)
			{
				...
				if(g_config.getString(ConfigManager::HOUSE_RENTASPRICE) == "yes" && house->getRent())
...
bool Houses::reloadPrices()
{
	for(HouseMap::iterator it = houseMap.begin(); it != houseMap.end(); ++it)
	{
		if(g_config.getString(ConfigManager::HOUSE_RENTASPRICE) == "yes" && it->second->getRent())
			continue;
			...
			if(depot)
			{
				if(g_config.getString(ConfigManager::HOUSE_NEED_PREMIUM) != "yes" || player->isPremium())
				{
					...
					if(g_game.removeMoney(depot, house->getRent(), FLAG_NOLIMIT))
						paid = true;
					else if(g_config.getString(ConfigManager::BANK_SYSTEM) == "yes" && player->balance >= house->getRent())
					{
