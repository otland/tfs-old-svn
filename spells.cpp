...
bool InstantSpell::SearchPlayer(const InstantSpell* spell, Creature* creature, const std::string& param)
{
	...
	g_game.addMagicEffect(player->getPosition(), NM_MAGIC_MAGIC_ENERGY);
	return true;
}
...
bool InstantSpell::Levitate(const InstantSpell* spell, Creature* creature, const std::string& param)
{
	...
	if(ret == RET_NOERROR)
	{
		g_game.addMagicEffect(player->getPosition(), NM_MAGIC_TELEPORT, player->isGhost());
		return true;
	}

	player->sendCancelMessage(ret);
	g_game.addMagicEffect(player->getPosition(), NM_MAGIC_POFF, player->isGhost());
	return false;
}
...
bool InstantSpell::Illusion(const InstantSpell* spell, Creature* creature, const std::string& param)
{
	...
	if(ret == RET_NOERROR)
		g_game.addMagicEffect(player->getPosition(), NM_MAGIC_MAGIC_BLOOD);
	else
	{
		player->sendCancelMessage(ret);
		g_game.addMagicEffect(player->getPosition(), NM_MAGIC_POFF);
