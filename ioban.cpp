...
bool IOBan::addIpBanishment(uint32_t ip, time_t banTime, std::string comment, uint32_t gamemaster)
{
	...
	DBQuery query;
	query << "INSERT INTO `bans` (`type`, `value`, `expires`, `added`, `admin_id`, `comment`, `param`) VALUES (" << (BanType_t)BANTYPE_IP_BANISHMENT << ", " << ip << ", " << banTime << ", " << time(NULL) << ", " << gamemaster << ", " << db->escapeString(comment.c_str()) << ", 4294967295)";
