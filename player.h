...
class Player : public Creature, public Cylinder
{
	public:
		...
		void setStaminaMinutes(uint32_t _stamina) {setStamina(
			_stamina * STAMINA_MULTIPLIER);}
		void addStamina(int64_t value) {stamina = std::min((int64_t)STAMINA_MAX,
			(int64_t)std::max((int64_t)0, int64_t(stamina + value)));}
		void removeStamina(int64_t value) {addStamina(-value); sendStats();}
		...
		void sendCreatureAppear(const Creature* creature, uint32_t stackpos)
			{if(client) client->sendAddCreature(creature, creature->getPosition(), stackpos);}
